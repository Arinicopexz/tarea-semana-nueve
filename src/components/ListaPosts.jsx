...
function ListaPosts() {
  ...
  useEffect(() => {
    const cargarPosts = async () => {
      try {
        ...
        if (usuarioSeleccionado) {
          url += `&userId=${usuarioSeleccionado}`;
        }
    ...
    cargarPosts();
  }, [pagina, limite, usuarioSeleccionado]);
  ...
  return (
    <div>
      ...
      <div className="filtro-usuarios">
        <label htmlFor="usuario-select">Filtrar por usuario:</label>
        <select
          id="usuario-select"
          value={usuarioSeleccionado}
          onChange={(e) => {
            setUsuarioSeleccionado(e.target.value);
            setPagina(1); // Resetear a página 1 al cambiar filtro
          }}
        >
